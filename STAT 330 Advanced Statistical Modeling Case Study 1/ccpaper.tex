...
We first consider the level 1 \emph{marginal residuals} for each of the predictors as well as fitted values. From \Cref{fig: mar resid v fit}, there are some slight curvature in the fitted values, but not a huge concern. We see a similar pattern in \texttt{Activity Levels} (\Cref{fig: mar resid v act}), but generally not a huge concern. \texttt{Time} and \texttt{Standing} looks fine (see \Cref{fig: mar resid v time} and \Cref{fig: mar resid v stand}). 

...
ggplot(nurse_filtered, aes(x = as.factor(snum), y = sys)) +
  ...
  labs(x = "Subject ID",
       y = "BP",
       title = "Random subset of BP by Subjects") +
...
ggplot(nurse_filtered, aes(x = timepass, y = sys, color = as.factor(snum))) + 
  ...
  geom_line(stat = "smooth", method = "lm", se = F, alpha = 0.8) +
    labs(x = "Minutes since first measurement",
       y = "BP",
       title = "Random subset of BP vs. time passed by Subjects",
       color = "Subject ID") +
  guides(color = "none")
...
ggplot(nurse_filtered, aes(x = mnact5, y = sys, color = as.factor(snum))) + 
  ...
  geom_line(stat = "smooth", method = "lm", se = F, alpha = 0.8) +
    labs(x = "Activity Level",
       y = "BP",
       title = "Random subset of BP vs. Activity Level",
       color = "Subject ID") +
  guides(color = "none")
...
ggplot(nurse_filtered, aes(x = as.numeric(standing), y = sys, color = as.factor(snum))) + 
  ...
  geom_line(stat = "smooth", method = "lm", se = F, alpha = 0.8) +
    labs(x = "Standing vs. Not Standing",
       y = "BP",
       title = "Random subset of BP vs. Posture (2 Levels)",
       color = "Subject ID") +
  guides(color = "none")
...
ggplot(nurse_filtered, aes(x = mood, y = sys, color = as.factor(snum))) + 
  ...
  geom_line(stat = "smooth", method = "lm", se = F, alpha = 0.8) +
    labs(x = "Mood",
       y = "BP",
       title = "Random subset of BP vs. Mood",
       color = "Subject ID") +
  guides(color = "none")
```
...
```{r RI with Level 1 and 2 Covariates}
nurse_lmm_RI_lv12 <- lmer(sys ~ timepass + mnact5 + standing + mood + 
                           day2 + age + fh_2 +
                       (1 | snum), data = nurse_filtered)

summary(nurse_lmm_RI_lv12)
anova(nurse_lmm_RI_lv1, nurse_lmm_RI_lv12)
```
